...
| Panel align to selection + distribute (V1 100%/0°, V3 100%/15°, V4 non-uniform, cropped) | WORKS | Saved Positions match the geometry to 0.01 px: Selection left/right → all three at the union's left −112 / right 1704.8; vcenter → all at 460. Distribute h-centers put the middle clip's centre at the mean of the outer two (834.8); v-centers 496; h-gaps / v-gaps gave equal gaps (−191 / −78.5, negative because the clips overlap); outer clips never moved and the other axis was untouched. Undo count and the < 3 clips refusal not yet reported. | `test_projects/p6_*.prproj`, 2026-09-26 |
