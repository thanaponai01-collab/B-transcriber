...
| Panel align to sequence frame (six buttons, rendered bounds of the cropped picture) | WORKS | Saved Position puts the bounds exactly on the frame edge or centre (0 / 960 / 1920, 0 / 540 / 1080), each click moving only its own axis. Checked on V1 (100%, 0°), V3 (100%, 15°: bounds 768·cos15 + 288·sin15 = 816.4 wide) and V4 (Uniform off, Scale 50 × Width 80: bounds 614.4 × 144). Bounds are by the panel's model; Program Monitor look on the six V1 clicks not reported. | `cutdeck.prproj_anchor_check` corners, `test_projects/a5_*.prproj`, 2026-09-26 |
| Panel anchor + align on an Adjustment Layer | WORKS | AL is handled like a clip (Motion, 1920×1080 source). At Scale 50: anchor bottom-right kept the picture put (0.00 px); align left put the left edge at 0. | `test_projects/gap_al2_*.prproj`, 2026-09-26 |
| Panel edit on a non-square-pixel clip (2.0 PAR) | WORKS | Anchor click is refused with "Skipped: \"Black Video Copy 01\": non-square pixels are not supported yet."; the before/after saves show no change. | `test_projects/gap_par_*.prproj`, 2026-09-26 |
| Panel align to selection + distribute (V1 100%/0°, V3 100%/15°, V4 non-uniform, cropped) | WORKS | Saved Positions match the geometry to 0.01 px: Selection left/right → all three at the union's left −112 / right 1704.8; vcenter → all at 460. Distribute h-centers put the middle clip's centre at the mean of the outer two (834.8); v-centers 496; h-gaps / v-gaps gave equal gaps (−191 / −78.5, negative because the clips overlap); outer clips never moved and the other axis was untouched. With V1+V3 only, distribute refused: "Select at least 3 clips to distribute." One Ctrl+Z undoes a whole 3-clip action (user's "yes"). | `test_projects/p6_*.prproj`, 2026-09-26 |
