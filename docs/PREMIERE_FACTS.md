...
| Graphic Source Text (`AE.ADBE Text` [0]) reads | ABSENT | `getStartValue()` resolves **empty** (no error). `getValueAtTime(t)` throws "not supported for these value types. Use GetKeyframeAtTime". `getKeyframePtr(t)` throws "Illegal Parameter type". So the font, size and string can't be read from the panel. The **saved `.prproj`** has them: gzip XML, the `ArbVideoComponentParam` named Source Text, `StartKeyframeValue` base64 (a binary blob holding the PostScript font name and the string). | Check Transform, live 2026-09-24 |
