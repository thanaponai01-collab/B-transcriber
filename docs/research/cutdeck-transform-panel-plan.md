...
| **2** | 🔨 **BUILT 2026-09-24, NOT RUN LIVE.** Numeric edit of those four, one clip at a time, with undo (`transform/apply.js`, `features/align.js setField`, inputs in `core/alignPanel.js`) | Research-doc gate 2 — write a point, read it back, confirm in Effect Controls *and* Program Monitor, undo |
| **3** | 🔨 **BUILT 2026-09-24 together with Phase 4, NOT RUN LIVE.** Nine-point anchor picker; built with preserve-position ON (the picture stays put), targets on the cropped picture | Phase 2. `P_new = P_old + R·S·(A_new − A_old)` is geometry reasoning, **not** an Adobe formula — it is a hypothesis Phase 3 tests. **They DO use different spaces** (Part 1a, second run: Position sequence-normalized, Anchor source-normalized), so both must be converted to pixels in their own frames before applying it |
| **4** | 🔨 **BUILT with Phase 3.** Preserve-position on; batch across a multi-clip selection, one undo step; keyframed and non-square clips skipped with a reason | Gate 4 — nine targets × {100%, nonuniform scale} × {0°, 90°, arbitrary} |
| **5** | 🔨 **BUILT 2026-09-24, NOT RUN LIVE.** Align to sequence frame: left/centre/right/top/middle/bottom, on rendered bounds (crop, scale incl. non-uniform, rotation) | Gate 3 **and** a decided answer for source dimensions. Expect "refuse clearly when unknown" |
